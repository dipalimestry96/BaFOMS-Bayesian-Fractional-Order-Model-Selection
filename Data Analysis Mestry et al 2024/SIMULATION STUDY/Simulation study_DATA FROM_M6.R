...
## Simulated data: we have simulated data from the model M6 and then using this
...
##################################################################################
#### 1. Simulated data Generation
set.seed(122)  # for reproducibility of results
setwd("specify path of local directory here")

## data simulation step from the model M1: Caputo logistic model
K_sim = 30000         # Carrying capacity
alpha_sim = 0.5       # order of FDE
r_sim = 0.4           # growth rate
sigma_sim = 350       # standard deviation
...
log_prior = function(theta){
  sum(dunif(theta[1], min = 0, max = 1, log = TRUE),
      ...
      dnorm(theta[5], 2000, sqrt(1000), log = TRUE))
}
...
post_model = rjmcmcpost(post.draw = list(draw1, draw2, draw3, draw4, draw5, draw6), 
                        ...
                        save.all = TRUE)
write.table(post_model$result$`Posterior Model Probabilities`, "post_mod_prob_M6_122.txt", )
write.table(post_model$progress$prb, "post_mod_progress_M6_122.txt", row.names = FALSE)
capture.output(post_model$result, file = "post_rjmcmc_result_M6_122.txt")
...
post_prob = read.table("post_mod_prob_M6_122.txt", header = TRUE)
post_prob_temp = c(post_prob[1:6,1])

post_progress = as.matrix(read.csv("post_mod_progress_M6_122.txt", header = TRUE, sep = " "))
...
# to save the plot
filename = paste0("POSTERIOR_MODEL_PROBABILITIES_BARPLOT_simulation study_M6.jpeg")
