#### simulation study: data generated using the M2 model with 5 different sets of parameters
...
seeds = 121:125 # used seeds for reproducibility of results
set = matrix(NA, ncol = 4, nrow = length(seeds)) # matrix for saving the different sets values
set[1,] = c(0.9, 0.35, 30000, 1000) # set 1
set[2,] = c(0.7, 0.45, 40000, 1200) # set 2
set[3,] = c(0.45, 0.5, 25000, 1300) # set 3
set[4,] = c(0.5, 0.9, 30000, 1400) # set 4
set[5,] = c(0.8, 0.2, 40000, 1200) # set 5

post_prob_M2 = matrix(NA, length(seeds), 6) # matrix for saving the posterior model probabilities set wise

## data simulation step from the model M2
for (w in 1:length(seeds)) {
  ...
  N = numeric(n)
  mu_0 = 3000 # mean population size for N[1]
  N[1] = rnorm(1,mean = mu_0, sd = sigma_sim)
  mu_star = N[1] + h*r_sim*N[1]*(1-N[1]/K_sim)
  N[2] = rnorm(1, mean = mu_star, sd = sigma_sim )
  for (i in 3:n) {
    mean = mean_M2(N[i-1], N[i-2],M_alpha, h, alpha_sim, r_sim, K_sim)
    N[i] = rnorm(1, mean = mean, sd = sigma_sim)
    
  }
  
  par(mfrow = c(1,1))
  plot(t, N, type = "b", col= "red", pch = 16, main = "Simulated data from M2")
  ...
  post_model = rjmcmcpost(post.draw = list(draw1, draw2, draw3, draw4, draw5, draw6), 
                          ...
                          save.all = TRUE)
  post_prob_M2[w,] = post_model$result$`Posterior Model Probabilities`
  print(post_prob_M2[w,])
  filename = paste0("post_M2_set_", w, ".txt")
  write.table(post_prob_M2[w,], filename, row.names = FALSE )
  
}
write.table(post_prob_M2, "posterior_prob_mat_M2.txt",row.names = FALSE) # writing output file in given local folder on system

prob_mat = read.table(file = "posterior_prob_mat_M2.txt", header = TRUE)
prob_mat = as.matrix(prob_mat)
round(Re(prob_mat), digits = 4)
