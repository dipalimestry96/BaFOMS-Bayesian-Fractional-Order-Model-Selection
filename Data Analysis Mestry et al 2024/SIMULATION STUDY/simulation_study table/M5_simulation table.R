#### simulation study: data generated using the M5 model with 5 different sets of parameters
...
seeds = 151:155 # used seeds for reproducibility of results
set = matrix(NA, ncol = 4, nrow = length(seeds)) # matrix for saving the different sets values
set[1,] = c(0.75, 0.25, 40000, 400) # set 1
set[2,] = c(0.82, 0.2, 65000, 300) # set 2
set[3,] = c(0.9, 0.3, 35000, 300) # set 3
set[4,] = c(0.6, 0.35, 35000, 250) # set 4
set[5,] = c(0.4, 0.55, 45000, 350) # set 5

post_prob_M5 = matrix(NA, length(seeds), 6) # matrix for saving the posterior model probabilities set wise

## data simulation step from the model M5
for (w in 1:length(seeds)){
  ...
  for (i in 2:(n-1)) {
    mean = mean_M5(N, t,i,M_alpha,h,alpha_sim, r_sim, K_sim)
    N[i+1] = rnorm(1, mean = mean, sd = sigma_sim  )
    
  }
  print(w)
  par(mfrow = c(1,1))
  plot(t, N, type = "b", col= "red", pch = 16, main = "Simulated data from M5")
  print(N)
  print(sum(N<0))
  ...
  post_model = rjmcmcpost(post.draw = list(draw1, draw2, draw3, draw4, draw5, draw6), 
                          ...
                          save.all = TRUE)
  post_prob_M5[w,] = post_model$result$`Posterior Model Probabilities`
  print(post_prob_M5[w,])
  filename = paste0("post_model_prob_M5_set_",w, ".txt")
  write.table(post_prob_M5[w,], filename, row.names = FALSE)
}
write.table(post_prob_M5, "posterior_prob_mat_M5.txt",row.names = FALSE) # writing output file in given local folder on system


post_mod_prob = read.table("posterior_prob_mat_M5.txt", header = TRUE)
