#### simulation study: data generated using the M6 model with 5 different sets of parameters
...
seeds = 161:165 # used seeds for reproducibility of results
set = matrix(NA, ncol = 4, nrow = length(seeds)) # matrix for saving the different sets values
set[1,] = c(0.5, 0.35, 50000, 300) # set 1
set[2,] = c(0.29, 0.5, 32000, 300) # set 2
set[3,] = c(0.7, 0.2, 20000, 200) # set 3
set[4,] = c(0.4, 0.25, 35000, 300) # set 4
set[5,] = c(0.6, 0.4, 30000, 350) # set 5

post_prob_M6 = matrix(NA, length(seeds), 6) # matrix for saving the posterior model probabilities set wise

## data simulation step from the model M6
for (w in 1:length(seeds)) {
  ...
  #hist(C1[,4], main = expression(mu[0]))
  #abline(v = mu_0, col= "red", lwd = 3, lty =2)
  ...
  post_model = rjmcmcpost(post.draw = list(draw1, draw2, draw3, draw4, draw5, draw6), 
                          ...
                          save.all = TRUE)
  post_prob_M6[w,] = post_model$result$`Posterior Model Probabilities`
  print(round(Re(post_prob_M6[w,]),digits = 5))
  filename = paste0("post_mod_pro_M6_set_",w,".txt")
  write.table(post_prob_M6[w,], filename, row.names = FALSE)
}
write.table(post_prob_M6, "posterior_prob_mat_M6.txt",row.names = FALSE) # writing output file in given local folder on system

post_mod_prob = read.table("posterior_prob_mat_M6.txt", header = TRUE)
