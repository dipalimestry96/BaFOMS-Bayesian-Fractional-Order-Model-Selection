#### simulation study: data generated using the M1 model with 5 different sets of parameters
...
seeds = 111:115  # used seeds for reproducibility of results
set = matrix(NA, ncol = 4, nrow = length(seeds))  # matrix for saving the different sets values
set[1,] = c(0.2, 1, 50000, 300) # set 1
set[2,] = c(0.1, 1.25, 20000, 250) # set 2
set[3,] = c(0.25, 1.35, 20000, 400) # set 3
set[4,] = c(0.15, 1.2, 30000, 300) # set 4
set[5,] = c(0.35, 0.7, 35000, 350) # set 5
post_prob_M1 = matrix(NA, length(seeds), 6) # matrix for saving the posterior model probabilities set wise

## data simulation step from the model M1
for (w in 1:length(seeds)) {
  ...
  N = numeric(n)
  mu_0 = 5000 # mean population size for N[1]
  ...
  post_model = rjmcmcpost(post.draw = list(draw1, draw2, draw3, draw4, draw5, draw6), 
                          ...
                          save.all = TRUE)
  post_prob_M1[w,] = post_model$result$`Posterior Model Probabilities`
  print(post_prob_M1[w,])
  filename = paste0("post_M1_set_", w, ".txt")
  write.table(post_prob_M1[w,], filename, row.names = FALSE )
}
write.table(post_prob_M1, "posterior_prob_mat_M1.txt",row.names = FALSE) # writing output file in given local folder on system

prob_mat = read.table(file = "posterior_prob_mat_M1.txt", header = TRUE)
