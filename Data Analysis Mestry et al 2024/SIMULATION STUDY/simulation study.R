...
setwd("specify path here to store outputs") # path to save outputs

## data simulation step from the model M1: Caputo logistic model
K_sim = 30000         # Carrying capacity
alpha_sim = 0.3       # order of FDE
r_sim = 1.1           # growth rate
...
post_model = rjmcmcpost(post.draw = list(draw1, draw2, draw3, draw4, draw5, draw6), 
                        ...
                        model.prior = c(1/6, 1/6, 1/6, 1/6, 1/6, 1/6),chainlength = 1e5, TM.thin = 10 , 
                        save.all = TRUE)
