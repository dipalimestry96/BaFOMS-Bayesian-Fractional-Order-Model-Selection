...
## One data set is extracted from GPDD and analysis are carried out for this dataset
## GPDD Main IDs = 200
## GPDD Main ID = 200 : # Mammal species: Castor Canadensis 
# Commonly known as the Beaver
# with a temporal extent of 51 years (1931-1981)
...
# path of local folder on system for saving the output
setwd("Specify path of local folder for saving outputs")

GPDD_ID = 200 # gpdd data Main IDs
...
data = subset(gpdd_data, MainID == GPDD_ID)$Population   # population abundance

N = data                   # population abundance
...
N_1 = N[1]     # mean for initial population abundance
max_N = max(N)    # maximum of population abundance
min_N = min(N)    # minimum of population abundance

Max = 3*max_N     # upper bound for the prior distribution parameter value for K


params = c("alpha", "r", "K", "tau")     # parameters to be estimated

log_prior = function(theta){   #log prior density function for parameters
...
## bijection and its inverse function for RJMCMC for transition between the model space
g = function(psi){ psi }
...
acf(C1[,1], main = expression(alpha))
acf(C1[,4], main = expression(r))
acf(C1[,3], main = expression(K))
acf(C1[,5], main = expression(tau))
...
acf(C2[,1], main = expression(alpha))
acf(C2[,4], main = expression(r))
acf(C2[,3], main = expression(K))
acf(C2[,5], main = expression(tau))
...
acf(C3[,1], main = expression(alpha))
acf(C3[,4], main = expression(r))
acf(C3[,3], main = expression(K))
acf(C3[,5], main = expression(tau))
...
jagsfit_M4 = jags(data = c('N', 't', 'n', 'h','N_1','min_N','Max'), inits, params,
                  n.iter = n_iter, model.file = "M4_RL.txt", n.chains = 3, n.thin = 50)
...
acf(C4[,1], main = expression(alpha))
acf(C4[,4], main = expression(r))
acf(C4[,3], main = expression(K))
acf(C4[,5], main = expression(tau))
...
acf(C5[,1], main = expression(alpha))
acf(C5[,4], main = expression(r))
acf(C5[,3], main = expression(K))
acf(C5[,5], main = expression(tau))
...
jagsfit_M6 = jags(data = c('N', 'n', 'h', 'N_1', 'min_N','Max'), inits, params,
                  n.iter = n_iter, model.file = "M6_Grunwald.txt", n.chains = 3, n.thin = 50)
...
######## autocorrelation between samples
acf(C6[,1], main = expression(alpha))
acf(C6[,4], main = expression(r))
acf(C6[,3], main = expression(K))
acf(C6[,5], main = expression(tau))
...
# storing the outputs of RJMCMC in the working director
...
## 2. Barplot of posterior model probabilities
library(ggplot2)
post_mod_prob_200 = as.numeric(Re(read.table("post_mod_prob_real_data_GPDD_ID_200.txt", header = TRUE)[1:6,1]))

Mod_lebels = c("M1", "M2", "M3", "M4", "M5", "M6")
df_200 = data.frame(Model = Mod_lebels,post_mod_prob_200)

Mod_labels = c(expression(M[1], M[2], M[3], M[4], M[5], M[6])) # model names in mathematical symbols

# barplot of posterior model probabilities for GPDD ID 9330
p1 = ggplot(data = df_200, aes(x = Model, y = post_mod_prob_200)) +
  labs(x = "Model", y = "Posterior Model Probability", title = "GPDD ID: 200") +
  geom_bar(stat = "identity", fill = "aquamarine4") +
  geom_text(aes(label = round(post_mod_prob_200, 4)), vjust = -0.3, size = 3.5) +
  theme_minimal() +
...
# to save the plot
ggsave("POST_MOD_PROB_GPDD_ID_200.jpeg", plot = p1, width = 8, height = 6, dpi = 600)
...
for (m in 1:6) {
  ...
  h1_200 = ggplot(df, aes(x = alpha, ..density..)) + 
  ...
  ggsave(filename, plot = h1_200, width = 6, height = 4, dpi = 400) # to save the plot
  
  
  # posterior histogram along with posterior density plot for parameter r
  h2_200 =ggplot(df, aes(x = r, ..density..)) + 
    geom_histogram(aes(y = ..density..),
  ...
  ggsave(filename, plot = h2_200, width = 6, height = 4, dpi = 400) # to save the plot
  
  # posterior histogram along with posterior density plot for parameter K
  h3_200 = ggplot(df, aes(x = K, ..density..)) + 
    geom_histogram(aes(y = ..density..),
  ...
  ggsave(filename, plot = h3_200, width = 6, height = 4, dpi = 400) # to save the plot
  
  # posterior histogram along with posterior density plot for parameter tau
  h4_200 = ggplot(df, aes(x = tau, ..density..)) + 
    geom_histogram(aes(y = ..density..),
  ...
  ggsave(filename, plot = h4_200, width = 6, height = 4, dpi = 400) # to save the plot
...
post_mod_prob_200 = as.numeric(Re(read.table("post_mod_prob_real_data_GPDD_ID_200.txt", header = TRUE)[1:6,1])) #posterior model probabilities
m = which.max(post_mod_prob_200)
filename = paste0("post_M",m, "_GPDD_ID_", GPDD_ID,".txt")
