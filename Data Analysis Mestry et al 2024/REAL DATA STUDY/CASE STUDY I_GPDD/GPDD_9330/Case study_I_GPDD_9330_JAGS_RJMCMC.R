...
## GPDD ID = 9330
## GPDD ID = 9330 : # avian species, namely Phalacrocorax carbo 
                    # Commonly known as the Great cormorant, GPDD Main ID: 9330
                    # with a temporal extent of 21 years (1971-1992)
                    # population corresponds to year 1973 was missing 
                    # missing population value is filled using mean of adjacent years population
...
data_temp = subset(gpdd_data, MainID == GPDD_ID)$Population
...
library(ggplot2)
post_mod_prob_9330 = as.numeric(Re(read.table("post_mod_prob_real_data_GPDD_ID_9330.txt", header = TRUE)[1:6,1]))

Mod_lebels = c("M1", "M2", "M3", "M4", "M5", "M6")
df_9330 = data.frame(Model = Mod_lebels,post_mod_prob_9330)
...
p1 = ggplot(data = df_9330, aes(x = Model, y = post_mod_prob_9330)) +
  labs(x = "Model", y = "Posterior Model Probability", title = "GPDD ID: 9330") +
  geom_bar(stat = "identity", fill = "aquamarine4") +
  geom_text(aes(label = round(post_mod_prob_9330, 4)), vjust = -0.3, size = 3.5) +
  theme_minimal() +
...
# to save the plot
ggsave("POST_MOD_PROB_GPDD_ID_9330.jpeg", plot = p1, width = 8, height = 6, dpi = 400)
...
for (m in 1:6) {
  ...
  h1_9330 = ggplot(df, aes(x = alpha, ..density..)) + 
  ...
  ggsave(filename, plot = h1_9330, width = 6, height = 4, dpi = 400) # to save the plot
  
  
  # posterior histogram along with posterior density plot for parameter r
  h2_9330 =ggplot(df, aes(x = r, ..density..)) + 
    geom_histogram(aes(y = ..density..),
  ...
  ggsave(filename, plot = h2_9330, width = 6, height = 4, dpi = 400) # to save the plot
  
  # posterior histogram along with posterior density plot for parameter K
  h3_9330 = ggplot(df, aes(x = K, ..density..)) + 
    geom_histogram(aes(y = ..density..),
  ...
  ggsave(filename, plot = h3_9330, width = 6, height = 4, dpi = 400) # to save the plot
  
  # posterior histogram along with posterior density plot for parameter tau
  h4_9330 = ggplot(df, aes(x = tau, ..density..)) + 
    geom_histogram(aes(y = ..density..),
  ...
  ggsave(filename, plot = h4_9330, width = 6, height = 4, dpi = 400) # to save the plot
...
post_mod_prob_9330 = as.numeric(Re(read.table("post_mod_prob_real_data_GPDD_ID_9330.txt", header = TRUE)[1:6,1])) #posterior model probabilities
m = which.max(post_mod_prob_9330)
filename = paste0("post_M",m, "_GPDD_ID_", GPDD_ID,".txt")
