...
## we repeated this process for 1000 times and using qunatile we have
...
library(rgpdd)
data_temp = subset(gpdd_data, MainID == 9330)$Population    # population data for GPDD ID 9330
data = c(data_temp[1:2], mean(data_temp[2:3]), data_temp[3:length(data_temp)])   ## missing value filled with mean of adjacent values
...
filename = paste0("95_per_Confidence_Interval_MATRIX_M1_", "GPDD_ID_9330", ".txt")
...
filename = paste0("95_per_Confidence_Interval_MATRIX_M2_", "GPDD_ID_9330", ".txt")
...
filename = paste0("95_Confidence_Interval_","M", "2", "_", "9330(mode)", ".jpeg")
ggsave(filename, plot = p, width = 8, height = 6, dpi = 600) # to save plot

###########################################################################
######### For M3: Bayesian Confidence interval ##############################

############## posterior distribution of parameters for model M3
filename = paste0("post_M3_GPDD_ID_", "9330", ".txt")
C3 = as.matrix(read.table(filename, header = TRUE))[,c(1,4,3,5)]   # posterior values of model (M3) parameters obtained using the JAGS
C3 = C3[seq(1, nrow(C3), by = 60),] # thinning after autocorrelation check
...
filename = paste0("95_per_Confidence_Interval_MATRIX_M3_", "9330", ".txt")
...
filename = paste0("95_per_Confidence_Interval_MATRIX_M4_", "9330", ".txt")
...
p = ggplot(df, aes(x = t, y = CI_mat[,2])) +
  ...
  guides(color = guide_legend(override.aes = list(linetype = c(1, 0), shape = c(NA, 16))),
         shape = "none",
         fill = guide_legend(override.aes = list(alpha = 0.45)))
filename = paste0("95_Confidence_Interval_","M", "4", "_", "9330(mode)", ".jpeg")
ggsave(filename, plot = p, width = 8, height = 6, dpi = 600) # to save plot


###########################################################################
######### For M5: Bayesian Confidence interval ##############################

############## posterior distribution of parameters for model M5
filename = paste0("post_M5_GPDD_ID_", "9330", ".txt")
C5 = as.matrix(read.table(filename, header = TRUE))[,c(1,4,3,5)]   # posterior values of model (M5) parameters obtained using the JAGS
C5 = C5[seq(1,nrow(C5), by = 1020),]   ## thinning after autocorrelation check
...
filename = paste0("95_per_Confidence_Interval_MATRIX_M5_", "9330", ".txt")
...
p = ggplot(df, aes(x = t, y = CI_mat[,2])) +
  ...
  guides(color = guide_legend(override.aes = list(linetype = c(1, 0), shape = c(NA, 16))),
         shape = "none",
         fill = guide_legend(override.aes = list(alpha = 0.45)))
filename = paste0("95_Confidence_Interval_","M", "5", "_", "9330(mode)", ".jpeg")
ggsave(filename, plot = p, width = 8, height = 6, dpi = 600) # to save plot


########################################################################
######### For M6: Bayesian Confidence interval ###########################

############## posterior distribution of parameters for model M6
filename = paste0("post_M6_GPDD_ID_", "9330", ".txt")
C6 = as.matrix(read.table(filename, header = TRUE))[,c(1,4,3,5)]   # posterior values of model (M6) parameters obtained using the JAGS
C6 = C6[seq(1,nrow(C6), by = 15), ]  # thinning after autocorrelation check
...
filename = paste0("95_per_Confidence_Interval_MATRIX_M6_", "9330", ".txt")
...
p = ggplot(df, aes(x = t, y = CI_mat[,2])) +
  ...
  guides(color = guide_legend(override.aes = list(linetype = c(1, 0), shape = c(NA, 16))),
         shape = "none",
         fill = guide_legend(override.aes = list(alpha = 0.45)))
filename = paste0("95_Confidence_Interval_","M", "6", "_", "9330(mode)", ".jpeg" )
ggsave(filename, plot = p, width = 8, height = 6, dpi = 600) # to save plot
