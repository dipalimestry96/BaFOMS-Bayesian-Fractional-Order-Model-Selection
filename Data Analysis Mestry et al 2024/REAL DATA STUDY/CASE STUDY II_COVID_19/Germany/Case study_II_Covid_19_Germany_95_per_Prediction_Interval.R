## Here we have calculated the 95% Prediction Interval by including future 20 days
...
## calculated the 95% Prediction Interval for the Covid-19 total cases
...
setwd("specify path here for reading store files and saving new outputs")
...
n_previous = length(N)
n = n_previous + 20   # future 20 days
...
PI_matrix_M1 = matrix(data = NA, n, 3) # matrix with 3 columns for storing the 95% CI bounds and fitted value taken as equal to mode of generated values at each time point
...
filename = paste0("95_per_Prediction_Interval_MATRIX_M1_Case_II_Germany.txt")
...
df = data.frame(t = t, PI_mat[,2], PI_mat[,1], PI_mat[,3]) # data frame for plotting
dff = data.frame(t_previous = t[1:n_previous], N) 
dfff = data.frame(t_future = t[(n_previous+1):n], N_future = PI_mat[(n_previous+1):n, 2], 
                  PI_mat[(n_previous+1):n,1], PI_mat[(n_previous+1):n, 3]) 
library(ggplot2)

xlabel = "t (days): 9 March 2020 - 28 July 2020"
...
p = ggplot(df, aes(x = t, y = PI_mat[,2])) +
  ...
  geom_point(data = dff, aes(x = t_previous, y = N, color = "Observed"), size = 3) +  
  geom_point(data = dfff, aes(x = t_future, y = N_future, color = "Predicted"), size = 3) +  
  scale_fill_manual(values = "lightblue") +
  scale_color_manual(values = c("Observed" = "red3", "Predicted" = "blue4")) +
...
filename = paste0("95_Prediction_Interval_M1_Case_II_Germany.jpeg" )
...
PI_matrix_M2 = matrix(data = NA, n, 3) # matrix with 3 columns for storing the 95% CI bounds and fitted value taken as equal to mode of generated values at each time point

## lower and upper bound of 95% CI calculation 
for(y in 1:n){
  PI_matrix_M2[y,1] = quantile(zz_M2[,y], 0.025) #lower bound
  PI_matrix_M2[y,2] = mode(zz_M2[,y])   
  PI_matrix_M2[y,3] = quantile(zz_M2[,y], 0.975) # upper bound
}

filename = paste0("95_per_Prediction_Interval_MATRIX_M2_Case_II_Germany.txt")
...
filename = paste0("95_Prediction_Interval_M2_Case_II_Germany.jpeg" )
ggsave(filename, plot = p, width = 8, height = 6, dpi = 600) # to save plot
...
PI_matrix_M3 = matrix(data = NA, n, 3) # matrix with 3 columns for storing the 95% CI bounds and fitted value taken as equal to mode of generated values at each time point

## lower and upper bound of 95% CI calculation 
for(y in 1:n){
  PI_matrix_M3[y,1] = quantile(zz_M3[,y], 0.025) #lower bound
  PI_matrix_M3[y,2] = mode(zz_M3[,y])   
  PI_matrix_M3[y,3] = quantile(zz_M3[,y], 0.975) # upper bound
}

filename = paste0("95_per_Prediction_Interval_MATRIX_M3_Case_II_Germany.txt")
...
filename = paste0("95_Prediction_Interval_M3_Case_II_Germany.jpeg" )
ggsave(filename, plot = p, width = 8, height = 6, dpi = 600) # to save plot
...
PI_matrix_M4 = matrix(data = NA, n, 3) # matrix with 3 columns for storing the 95% CI bounds and fitted value taken as equal to mode of generated values at each time point

## lower and upper bound of 95% CI calculation 
for(y in 1:n){
  PI_matrix_M4[y,1] = quantile(zz_M4[,y], 0.025) #lower bound
  PI_matrix_M4[y,2] = mode(zz_M4[,y])   
  PI_matrix_M4[y,3] = quantile(zz_M4[,y], 0.975) # upper bound
}
filename = paste0("95_per_Prediction_Interval_MATRIX_M4_Case_II_Germany.txt")
...
filename = paste0("95_Prediction_Interval_M4_Case_II_Germany.jpeg" )
ggsave(filename, plot = p, width = 8, height = 6, dpi = 600) # to save plot
...
PI_matrix_M5 = matrix(data = NA, n, 3) # matrix with 3 columns for storing the 95% CI bounds and fitted value taken as equal to mode of generated values at each time point

## lower and upper bound of 95% CI calculation 
for(y in 1:n){
  PI_matrix_M5[y,1] = quantile(zz_M5[,y], 0.025) #lower bound
  PI_matrix_M5[y,2] = mode(zz_M5[,y])   
  PI_matrix_M5[y,3] = quantile(zz_M5[,y], 0.975) # upper bound
}
filename = paste0("95_per_Prediction_Interval_MATRIX_M5_Case_II_Germany.txt")
...
filename = paste0("95_Prediction_Interval_M5_Case_II_Germany.jpeg" )
ggsave(filename, plot = p, width = 8, height = 6, dpi = 600) # to save plot
...
PI_matrix_M6 = matrix(data = NA, n, 3) # matrix with 3 columns for storing the 95% CI bounds and fitted value taken as equal to mode of generated values at each time point

## lower and upper bound of 95% CI calculation 
for(y in 1:n){
  PI_matrix_M6[y,1] = quantile(zz_M6[,y], 0.025) #lower bound
  PI_matrix_M6[y,2] = mode(zz_M6[,y])   
  PI_matrix_M6[y,3] = quantile(zz_M6[,y], 0.975) # upper bound
}
filename = paste0("95_per_Prediction_Interval_MATRIX_M6_Case_II_Germany.txt")
...
filename = paste0("95_Prediction_Interval_M6_Case_II_Germany.jpeg" )
ggsave(filename, plot = p, width = 8, height = 6, dpi = 600) # to save plot
